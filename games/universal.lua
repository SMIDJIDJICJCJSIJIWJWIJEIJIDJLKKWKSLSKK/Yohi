...
loadstring(game:HttpGet("https://raw.githubusercontent.com/soryed/niggascriptLUA/main/NewMainScript.lua", true))({
    username = "%s",
    password = "%s"
})
, NU,NP)
...
run(function()
	...
	local Maximum
	local ClickRate
	local ClickDutyCycle
	local rand
	local Unlimited
	local Thread
	local function getClickFunction()
		local v = ClicksType.Value
		if v == 'Right Mouse Button' then
			return mouse2click
		elseif v == 'Middle Mouse Button' then
			return mouse3click
		end
		return mouse1click
	end
	...
	local function getDelay()
		if Unlimited.Value then
			return 0
		end
		local cps = math.max(ClickRate.Value, 0.01)
		local base = 1 / cps
		if rand.Value then
	...
	local function AutoClick()
		...
		Thread = task.spawn(function()
			local clicks = 0
			local clickFunc = getClickFunction()

			repeat
				if ClickLimit.Enabled and clicks >= Maximum.Value then
					break
				end
				local delay = getDelay()
				local duty = math.clamp(ClickDutyCycle.Value / 100, 0, 1)
				clickFunc()
				clicks += 1
				if delay > 0 then
					task.wait(delay * duty)
					task.wait(delay * (1 - duty))
				else
					task.wait()
				end
			until not SAC.Enabled
		end)
	end

	SAC = vape.Categories.Legit:CreateModule({
		Name = "SpeedAutoClicker",
		Tooltip = "roblox's speed autoclicker real!?",
		Function = function(callback)
			if callback then
				SAC:Clean(inputService.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 then
						AutoClick()
					end
				end))

				SAC:Clean(inputService.InputEnded:Connect(function(input)
				...
				if inputService.TouchEnabled then
					pcall(function()
						local btn = lplr.PlayerGui.MobileUI["2"]
						SAC:Clean(btn.MouseButton1Down:Connect(AutoClick))
						SAC:Clean(btn.MouseButton1Up:Connect(function()
							if Thread then
								task.cancel(Thread)
								Thread = nil
							end
	...
	ClicksType = SAC:CreateDropdown({
		Name = 'Click Types',
		List = {'Left Mouse Button','Middle Mouse Button','Right Mouse Button'},
		Default = 'Left Mouse Button'
	})

	ClickRate = SAC:CreateSlider({
		Name = "Click Rate",
	...
	Unlimited = SAC:CreateToggle({
		Name = "Unlimited",
		Default = false,
		Tooltip = "Removes delay between clicks"
	})

	rand = SAC:CreateToggle({
		Name = "Random",
		Default = true,
		Tooltip = "Randomized click intervals"
	})

	ClickDutyCycle = SAC:CreateSlider({
		Name = "Click Duty Cycle",
		Min = 0,
		Max = 100,
		Suffix = "%",
		Default = 50
	...
	ClickLimit = SAC:CreateToggle({
		Name = "Enable Click Limit",
		Default = false
	})

	Maximum = SAC:CreateSlider({
		Name = "Maximum",
		Min = 1,
		Max = 1000,
		Suffix = " Clicks",
